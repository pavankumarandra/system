...
def normalize_fd(msg):
    """
    Preprocessor to force ASCWriter to always use 'd <len>' instead of 'dlc <len>'
    for CANFD frames.
    """
    if getattr(msg, "is_fd", False):
        # Keep the CANFD marker but avoid ASCWriter switching to 'dlc'
        msg.is_fd = False
    return msg
...
class CANLogger:
    ...
    def start(self, filename=None):
        ...
        try:
            ...
            # Use ASCWriter + preprocessor to normalize FD length output
            self.writer = ASCWriter(self.file)
            self.notifier = can.Notifier(self.bus, [self.writer], preprocessors=[normalize_fd])
