                ...
                try:
                    ...
                    with Client(self.active_conn["conn"], request_timeout=2, config=self.active_conn["client_config"]) as client:
                        ...
                        if service_int == 0x10:
                            ...
                            try:
                                # Handle valid subfunction case (e.g., 0x01)
                                if subfunc.strip():
                                    # Support multi-byte subfunctions (e.g., "01 02")
                                ...
                                # Handle empty subfunction case (invalid message)
                                else:
                                    subfunc_bytes = b''
                                ...
                                # Evaluate response
                                if response:
                                    logging.info(f"{tc_id} - Received: {response.hex().upper()}")
                                    expected_bytes = [int(b, 16) for b in expected.strip().split()]
                                    if response[:len(expected_bytes)] == bytes(expected_bytes):
                                        logging.info(f"{tc_id} - {step_desc} -> PASS")
                                    else:
                                        logging.warning(f"{tc_id} - {step_desc} -> FAIL - Mismatch response")
                                else:
                                    logging.warning(f"{tc_id} - No response received")

                            except ValueError as ve:
                                logging.error(f"{tc_id} - Hex Error: {ve}")
                                oled.display_centered_text(f"{tc_id}\nHex Error")
                            except Exception as e:
                                logging.error(f"{tc_id} - Exception: {type(e).__name__} - {str(e)}")
                                oled.display_centered_text(f"{tc_id}\nError: {str(e)[:16]}")
                            ...
                            try:
                                ...
                                if subfunc_clean:
                                    # Validate hex string length (should be exactly 6 hex digits i.e., 3 bytes)
                                    if len(subfunc_clean) % 2 != 0:
                                        subfunc_clean = "0" + subfunc_clean  # pad if odd-length hex string

                                    subfunc_bytes = bytes.fromhex(subfunc_clean)
                                    if len(subfunc_bytes) != 3:
                                        logging.warning(f"{tc_id} - Invalid DTC group length: {len(subfunc_bytes)} bytes")
                                else:
                                    # Empty subfunc: just the service byte (e.g., "14")
                                    subfunc_bytes = b''
                                ...
                                if response_data:
                                    ...
                                    if expected:
                                        ...
                                        logging.info(f"{tc_id} - No expected value provided for verification.")
                                else:
                                    logging.warning(f"{tc_id} - No response received")
                            ...
                            try:
                                ...
                                if subfunc.strip():  # If subfunction provided
                                    parts = subfunc.strip().split()

                                    if len(parts) == 2:
                                        # Case like "01 01" or "02 03"
                                        control_type = int(parts[0], 16)
                                        comm_type = int(parts[1], 16)
                                        raw_request += bytes([control_type, comm_type])
                                        logging.info(f"{tc_id} - Parsed subfunc as split: controlType={control_type:02X}, commType={comm_type:02X}")
                                    else:
                                        # Case like "0101", "0003", etc.
                                        subfunc_clean = subfunc.strip().replace(" ", "")
                                        if len(subfunc_clean) % 2 != 0:
                                            subfunc_clean = "0" + subfunc_clean  # Pad if needed
                                        subfunc_bytes = bytes.fromhex(subfunc_clean)
                                        raw_request += subfunc_bytes
                                        logging.info(f"{tc_id} - Parsed subfunc as hex string: {subfunc_bytes.hex().upper()}")
                                else:
                                    # Empty subfunction: sends only service byte (invalid case)
                                    logging.warning(f"{tc_id} - Subfunction is empty, sending only service byte")

                                # Send the constructed raw request
                                ...
                                if response_data:
                                    ...
                                    if expected:
                                        try:
                                            expected_bytes = bytes([int(b, 16) for b in expected.strip().split()])
                                            if response_data.startswith(expected_bytes):
                                                logging.info(f"{tc_id} - {step_desc} -> ✅ PASS")
                                            else:
                                                logging.warning(f"{tc_id} - {step_desc} -> ❌ FAIL - Expected {expected_bytes.hex().upper()}, got {response_hex}")
                                        except Exception as e:
                                            logging.error(f"{tc_id} - Error parsing expected data: {e}")
                                    else:
                                        logging.info(f"{tc_id} - No expected response provided for validation.")
                                else:
                                    logging.warning(f"{tc_id} - No response received.")

                            except ValueError as ve:
                                logging.error(f"{tc_id} - Invalid hex in subfunction: '{subfunc}' -> {ve}")
                            except Exception as e:
                                logging.error(f"{tc_id} - Exception in CommunicationControl handling: {e}")
