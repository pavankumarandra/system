            ...
            for step in steps:
                ...
                try:
                    ...
                    with Client(self.active_conn["conn"], request_timeout=2, config=self.active_conn["client_config"]) as client:
                        ...
                        if service_int == 0x10:
                            try:
                                # Prepare subfunction bytes (can be empty or multi-byte)
                                ...
                                if response_data:
                                    ...
                                    logging.warning(f"{tc_id} - No response received (timeout or empty)")

                            except ValueError as ve:
                                logging.error(f"{tc_id} - Invalid hex format: {str(ve)}")
